...
$config = Join-Path $repoRoot "configs\wb_microjepa_0m_braingraph_targeted_contrast.yaml"
$runDir = Join-Path $repoRoot "runs\wb_microjepa_0m_braingraph_targeted_contrast"
$summaryOut = Join-Path $repoRoot "runs\wb_microjepa_0m_frontier_compare.csv"
...
& $python -m src.wb_microjepa.compare_frontier `
  ...
  (Join-Path $repoRoot "runs\wb_microjepa_0l_braingraph_specialization_loss") `
  --output $summaryOut
